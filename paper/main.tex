...
% and appendices): 9,188 words. Counted with paper/wordcount.py, which strips
% the preamble, the abstract, all float environments, the appendices and the
% bibliography before counting, and which charges one word per \cite (the
% conservative convention; superscript citation numerals arguably count as
% none, which gives 9,139). Section-by-section: Introduction 841, Theory 1282,
% Methods 1676, Results 2182, What did not work 1427, Relation to prior work
% 749, Scope and limitations 415, Conclusion 366, Supplementary material 57.
%
...
\begin{abstract}
% 247 words (counted by paper/wordcount.py).
...
observable $A$ at inverse temperature $\beta$, with error field $\delta U$
(fitted minus reference potential), perturbation theory gives
$\Delta\langle A\rangle = -\beta\,\mathrm{Cov}_0(A,\delta U)$: the shift in a
static observable is minus $\beta$ times the covariance, under the reference
...
Lennard-Jones reference, where $\delta U$ is known everywhere, four error fields
matched in force RMSE to 0.5\% shift a target pair count from $0.3\sigma$ to
$16.2\sigma$, ordered by the Pearson correlation $\rho(A,\delta U)$. Predicted
from reference-ensemble samples alone, with no surrogate simulation, those
shifts match measurement to $1.01\sigma$ over eight designed fields and
$1.06\sigma$ over ten fitted models. Force error ranks models well across three
decades (Spearman $\rho = 0.83$ [0.67, 0.93], $n = 34$) but is unresolved from
zero inside a factor-2.6 band where the observable error still spans $30\times$
($\rho = 0.34$ [$-0.26$, 0.77]) while the response prediction retains
$\rho = 0.94$ [0.76, 1.00], both $n = 15$: a coarse filter, not a selector. That
refutes one of our predictions; two more fail: our smoothness-ratio metric
carries no information ($\rho = -0.02$ [$-0.37$, 0.35], $n = 34$), and our
$w^{3/2}$ width scaling measures exponent 0.56. Scope: one
...
$\nabla\dU$. A linear functional of a function and a quadratic functional of its
gradient are related by an inequality and by nothing stronger. The inequality is
loose, and Sec.~\ref{sec:results} shows how loose.

Testing that claim requires knowing $\dU$, which in ordinary practice is exactly
...
the point. Because $U_0$ is known everywhere in configuration space rather than
on a test set, $\dU$ is exactly computable at every sampled configuration,
reference observables converge to arbitrary precision so that a discrepancy is a
property of the surrogate rather than of the reference, and --- decisively ---
...
into a constructive one: we can build models that a force-error report cannot
...
held-out forces have reported that force error and simulation quality come
apart\cite{Fu2023, Stocker2022}, and the recommendation to validate on
simulation observables is now explicit in the methodological
literature\cite{Morrow2023}. Practitioners have also reported specific failures
that a force RMSE does not anticipate\cite{Kovacs2021, Deng2025, Pota2024}. The
statistical machinery we use is equally standard: the identity below is
...
averages\cite{Imbalzano2021}. We claim no novelty for it, here or anywhere
below. What we claim is narrower and we state it defensively: the reading of
...
falsification at force RMSE matched to 0.5\%; and the decomposition of the
...
\item A quantitative validation of that prediction on an analytic reference:
$1.01\sigma$ root-mean-square residual over eight designed error fields
($n = 8$) and $1.06\sigma$ over ten fitted models ($n = 10$), at a cost of one
energy evaluation per stored frame per model and no simulation with the
surrogate (Secs.~\ref{sec:prediction} and \ref{sec:fitted}).
\item A constructive counterexample set: four error fields matched in force RMSE
to 0.5\%, whose target-observable shifts run from $-0.220 \pm 0.640$ to
$+9.891 \pm 0.611$ pairs, ordered by $\rho(A,\dU)$
(Sec.~\ref{sec:counterexamples}).
\item A decomposition of force RMSE's value as a ranking metric: strong across
three decades, unresolved within a narrow band where the physics still varies by
$30\times$ (Sec.~\ref{sec:regimes}). This refutes our own pre-registered
prediction and replaces it with a more useful one.
\item Three refuted predictions of our own, reported as results rather than as
caveats (Sec.~\ref{sec:failures}), together with a demonstrated boundary of
validity of the linear estimator and an open, unresolved 35\% discrepancy
...
was a slowly melting crystal, wrong in energy by 30--40\%, and invisible to
every sampler diagnostic we had (Sec.~\ref{sec:equilibration}).
\end{enumerate}

Section~\ref{sec:failures}, ``What did not work,'' is not an appendix and not a
limitations paragraph. Three predictions made in this study were refuted by this
study: a $w^{3/2}$ scaling of observable error with the width of the error
...
models \emph{well} across decades and poorly only inside a narrow band --- and
the corrected statement is stronger and more useful than the one we set out to
demonstrate.
...
above all not the correlation, which is the factor that separates a harmless
model from a harmful one and which appears nowhere in standard practice.

\emph{(ii) It needs only reference-ensemble samples.} Every expectation in
Eq.~\eqref{eq:linear} is taken under $\avgz{\cdot}$. Given a stored reference
...
evaluation per stored frame per model. For screening many models against many
observables this is a large saving, and it is also what makes the claim
falsifiable, since the prediction and the direct measurement are then separate
experiments.

\emph{(iii) The second-order term is a computable warning light.} The $n = 2$
term is estimable from the same samples. Its ratio to the first-order term is a
...
reasoning about it is safe --- including the field's own implicit assumption
that a little more accuracy helps a little. We report this diagnostic wherever
it was recorded, and report its absence where it was not.

\subsection{Vector observables, and the contrast with free energy}

The radial distribution function is not a scalar but a vector of bin
...
natural summary of ``how wrong is the structure'' is then a norm of that vector
--- but the norm is taken \emph{after} the projection, which is the opposite
...
\emph{covariances}. A model can therefore have an excellent free energy and poor
...
error field in an orthonormal basis $\{\varphi_k\}$ labelled by spatial
frequency, $\dU = \sum_k c_k \varphi_k$. Then
\begin{align}
...
covariance in Eq.~\eqref{eq:modecov} applies no such weight, and because
...
same error spectrum in opposite directions, which produces two failure modes
...
slightly wrong well depth, a bias from an under-covered region of the training
distribution: nearly free in force RMSE, and able to move observables. The
second is the dangerous direction, and it is the error structure that
regularised fits and smooth kernels prefer to produce.

\subsection{A heuristic, retracted in advance}
\label{sec:retraction}

It is tempting to make Sec.~\ref{sec:frequency} quantitative with a
one-parameter family. Perturb a pair potential by a Gaussian shell bump of
...
scales as $w$, giving $\mathrm{RMSE}_F \sim a/\sqrt{w}$; its coupling to the
radial distribution function is an integral over the shell,
$|\Cov_0(g,\dU)| \sim a\,w$. At fixed force error the ratio would then grow as
\begin{equation}
...
advance of the measurement. Both inputs have regimes of validity that a real
system leaves quickly. Counting pairs in the shell as $\propto w$ holds only
while $w$ is small compared with the scale on which $g(r)$ varies, which near
the first peak of liquid argon is a few tenths of an \AA ngstr\"om, so the
estimate is already marginal at $w = 0.3$~\AA. And treating the observable as
...
survives, and what we do test, is the weaker statement that force error and
...
varying by a factor of 4.7 at fixed force error, and Sec.~\ref{sec:w32} reports
the measured exponent, 0.56, against the 1.5 of Eq.~\eqref{eq:w32}.

\subsection{What the theory does not cover}
\label{sec:dynamics}

Everything above concerns static averages. Transport coefficients are different
in kind: perturbing $U_0 \to U_0 + \dU$ changes both the ensemble the average is
...
$\Cov_0(A,\dU)$. There is independent literature support for the asymmetry ---
Wu \emph{et al.}\cite{Wu2024} derive a correction to lattice thermal
conductivity that depends on a \emph{norm} of the force error, which is exactly
what one expects when $\delta F$ acts through the dynamics rather than through a
projection. The experiment designed to test our conjecture, a rank correlation
...
The reference system is Lennard-Jones argon\cite{Jones1924}, $N = 108$ atoms in
a cubic cell with periodic boundary conditions at number density
$\rho = 0.0200$~\AA$^{-3}$ and temperature $T = 120$~K; in reduced units
...
HansenMcDonald2013}. The parameters are $\epsilon = 0.0103$~eV,
$\sigma = 3.405$~\AA, cutoff $7.0$~\AA, in the \emph{shifted-force} form.

The cutoff mode is not cosmetic. Table~\ref{tab:cutoff} in
Appendix~\ref{app:validation} gives the energy and force discontinuities of the
four available modes measured at $r_c \mp 10^{-7}$~\AA. A merely shifted
potential has a continuous energy and a force discontinuity of
$1.8\times10^{-4}~\eVA$, which delivers an impulse every time a pair crosses the
cutoff. This study measures small systematic differences between ensembles; an
artefact of that kind would not be distinguishable from the effect under
investigation.
...
\emph{regardless of integration error}, which appears only as a reduced
acceptance rate and never as a biased distribution.

That property is the reason for the choice, and it forecloses an objection that
would otherwise be fatal: a thermostat sampling something slightly other than
the canonical distribution would introduce exactly the kind of small systematic
...
burn-in to an acceptance of 0.75, giving $\approx 30$~fs after adaptation. An
independent single-particle Metropolis sampler\cite{Metropolis1953} sharing no
code beyond the potential is included as a cross-check.
...
$\avg{U} = 0.41462 \pm 0.00196$~eV against the exact $(3N/2)k_BT = 0.41363$~eV
($0.5\sigma$), and $\Var(U) = 0.003459$~eV$^2$ against the exact
$(3N/2)(k_BT)^2 = 0.003564$~eV$^2$ (Table~\ref{tab:validation}). Both moments
matter, because an incorrect temperature scale reproduces the mean and fails on
the variance.
...
On the original protocol the global Steinhardt bond-order
...
to $-0.043$~eV/atom, so the reference ensemble was wrong in energy by 30--40\%.
...
distribution. Those are different questions and only the second bears on an
ensemble average. We record the failure at length because it is invisible in
exactly the way that matters: every quantity in this paper is a small difference
between two ensembles, and an unrelaxed reference ensemble does not announce
itself --- it simply moves the answer.

The protocol now used melts at $5\times$ the target temperature for a few
hundred proposals, anneals to the target temperature, and refuses to return a
configuration with $Q_6 > 0.20$. Melting at the target temperature also works
but is an order of magnitude slower, because the barrier to be crossed is
precisely the one that makes the crystal metastable. A 108-atom liquid gives
$Q_6 \approx 0.08$ and perfect fcc gives 0.5745, so the threshold is far from
both. After the protocol, $Q_6 = 0.074$ and $U/N = -0.0315$~eV with no drift
between the halves of the production run. Every production trajectory is then
split in half and both its energy and its order parameter are compared between
halves; a drift exceeding three combined standard errors \emph{raises} rather
than warns, because a failure mode with no outward sign will eventually be
ignored if it is only a warning. The quick-run mode that shrinks every other
size parameter for smoke tests deliberately never shortens equilibration.

\subsection{Observables}

The measured quantities are pair counts in radial bins,
$A_k(x) = |\{(i,j) : r_k \le r_{ij} < r_{k+1}\}|$, on eight bins spanning 3.0 to
7.0~\AA. These differ from the radial distribution function $g(r)$ in that bin only by a
constant at fixed $N$ and $V$\cite{AllenTildesley2017}, and the conversion is
provided, but the analysis is done on the counts.
The reason is interpretability rather than convenience: response theory applies
to an observable that is a plain function of configuration, and a bin count is
exactly that, so no normalisation stands between the predicted quantity and the
measured one.

The bins taken together form a vector observable. The designed-perturbation arm
additionally targets a \emph{scalar}: the pair count in a single bin, 3.4 to
3.9~\AA, across the first peak. That matters statistically, since it makes the
covariance a $K$-vector rather than a $K\times J$ matrix and so estimable from
far fewer frames. It is also a limitation, stated here as well as in
Sec.~\ref{sec:scope}: the headline demonstration of
Sec.~\ref{sec:counterexamples} concerns a single-bin observable, and
Sec.~\ref{sec:orthogonality} shows directly that the result does not transfer to
the other bins of the same curve.
...
finite differences to $\sim 10^{-10}$ on both cubic and sheared triclinic cells.

The constructions used here expand a pair error field in a basis of Gaussian
shell bumps, which makes the covariance with an observable \emph{linear} in the
...
and yields three families: \emph{null}, whose first-order predicted effect on
the target observable is zero by construction; \emph{aligned}, of maximal effect
per unit force error; and \emph{random}, the control. All are then scaled to the
same force RMSE on the reference ensemble, so the number a practitioner would
report is held fixed across the comparison.

Out-of-sample discipline is essential and we state it explicitly. A null field
is orthogonal to the observable \emph{on the frames it was built from} by
definition, so the construction is worthless unless it survives on fresh frames.
The reference trajectory is split: fields are constructed on one half, and every
reported number --- force RMSE, predicted shift, covariance, measured shift ---
is computed on the other. How many construction frames are needed before the
construction generalises is itself measured and reported
(Table~\ref{tab:budget}), because with a badly estimated covariance the null
space found is the null space of the noise.
...
message-passing network\cite{Batzner2022, Gilmer2017} at two initialisation
seeds (\texttt{egnn\_c8}). These are deliberately small surrogates of a
classical potential, not the published potentials\cite{Bartok2010,
Batatia2022, Schutt2018, Thompson2015} whose evaluation motivates the work; the
architectures are cited as background, not as systems evaluated here. A
Gaussian-approximation-potential-style kernel
model\cite{Bartok2010} is implemented in the same codebase but is not included
in the reported zoo.

\subsection{Uncertainty estimation}
\label{sec:uncertainty}

Every uncertainty in this paper is a \textbf{standard error} obtained from
correlated-sample statistics. The symbol $\pm$ always denotes a standard error
and never a standard deviation, and $1/\sqrt{M}$ is never used: Monte Carlo
samples are correlated, and treating $M$ frames as $M$ independent samples
understates the error by roughly $\sqrt{2\tau_{\rm int}}$, a factor of several
for a typical liquid observable.

Means use Flyvbjerg--Petersen blocking\cite{Flyvbjerg1989}, with the plateau
taken over levels retaining at least eight blocks. Covariances, correlations and
fitted slopes use a moving-block bootstrap\cite{Kunsch1989, PolitisRomano1994}
with block length set to four integrated autocorrelation times of the
\emph{product} series that carries the error, $(A_m - \bar A)(\dU_m -
\overline{\dU})$; using the observable's own correlation time underestimates the
block length whenever $\dU$ varies slowly, which is precisely the
systematic-error case this paper is about. A jackknife with a different bias
structure is available as a cross-check; Appendix~\ref{app:statistics} gives the
estimators and their validation in full. Interval estimates from
\texttt{results/exp05\_proxy\_correlation} use 2000 bootstrap resamples;
intervals computed for this manuscript by the deposited script
\texttt{scripts/compute\_band\_correlations.py} use 2000 resamples with seed
20240517.

Every correlation is reported as $\rho = \text{value}\;[\text{low},
\text{high}]$ with its $n$. Significance is quoted as a $\sigma$-distance,
$|\text{value}|/\text{error}$, to one decimal place; no $p$-values are used.
Uncertainties are quoted to two significant figures and central values to the
same decimal place, with two deliberate exceptions: shifts in pair counts are
given throughout at the three-decimal precision of the deposited records, so
that every one of them can be checked against the JSON without rounding; and
exactness checks (Appendix~\ref{app:validation}) carry their full digits and are
labelled as such.
Prediction residuals quoted in $\sigma$ are the difference between the measured
and predicted shift divided by the standard error of the \emph{measurement}.
...
inflated by noise: $\mathbb{E}|v_{\rm meas}|^2 = |v_{\rm true}|^2 +
\mathbb{E}|v_{\rm noise}|^2$. All correlations reported in this paper use the
noise-subtracted series $|v_{\rm true}| = (\,|v_{\rm meas}|^2 - \mathbb{E}|v_{\rm
noise}|^2)^{1/2}$, floored at zero. The subtraction changes every reported
$\rho$ by less than 0.01, but it is necessary at the low-force-error end, where
signal and noise are comparable and the raw norm is almost entirely noise. The
raw-series values are given in a footnote to Table~\ref{tab:regimes} as a
robustness check.
...
The five experiment runs reported here total 13{,}311~s of wall time, about
3.7~h, on a four-core x86-64 Linux machine (Python 3.11.15, NumPy 2.4.6, SciPy
1.17.1, PyTorch 2.13.0+cpu). The manifests record the core count and the
platform string but not the processor model, so we do not state one. The
...
in the bin from 3.4 to 3.9~\AA, and each was scaled to a prescribed force RMSE.
Two levels were run, nominally $1.0\times10^{-3}$ and $4.0\times10^{-3}~\eVA$.
Measured out of sample --- on the half of the reference trajectory not used for
the construction --- the four fields agree in force RMSE to 0.5\% at each level --- a full spread of
0.52\% at both --- running from $0.99535$ to $1.00054 \times 10^{-3}~\eVA$ at
the lower level and $3.9814$ to $4.0022\times10^{-3}~\eVA$ at the upper.
Table~\ref{tab:counterexamples} gives
the full comparison and Fig.~\ref{fig:mechanism}(a) plots it.

The measured shifts in the target bin at the upper level run from
$-0.220 \pm 0.640$ pairs for the null field, which is $0.3\sigma$ from zero, to
$+9.891 \pm 0.611$ pairs for the aligned field, which is $16.2\sigma$ from zero.
The two random controls sit between them, at $-1.482 \pm 0.579$ ($2.6\sigma$)
and $+5.320 \pm 0.589$ ($9.0\sigma$). At the lower level the same four fields
give $-0.547 \pm 0.580$ ($0.9\sigma$), $-1.450 \pm 0.576$ ($2.5\sigma$),
$+0.835 \pm 0.589$ ($1.4\sigma$) and $+1.835 \pm 0.579$ ($3.2\sigma$). We
emphasise that the lower-level measurements are not the upper-level ones divided
by four: the \emph{predictions} scale almost exactly with the force level (2.552
against 10.206 pairs for the aligned field) because they are linear in $\dU$,
but the measurements at the lower level are noisier relative to their own signal
and land at 1.835 rather than at a quarter of 9.891.

Two conclusions follow, and neither is an interpretation. First, the reported
metric cannot distinguish these four models. Their force RMSE agrees to 0.5\%
out of sample; any procedure that selects a model by force error would rank them
as equivalent; one of them leaves the target observable untouched and another
moves it by sixteen standard errors. Second, the quantity that does distinguish
them is the correlation $\rho(A,\dU)$. Ordering the four fields by $\rho$ ---
random seed 1 at $+0.110$, null at $+0.009$, random seed 0 at $-0.430$, aligned
at $-0.582$ --- orders the measured shifts monotonically at both force levels,
and the relation is close to linear [Fig.~\ref{fig:mechanism}(a)], which is what
$\Delta\avg{A} = -\beta\,\sigma_0(A)\,\sigma_0(\dU)\,\rho_0$ requires when the
first two factors are held nearly fixed.
...
molecular dynamics and no Monte Carlo with any surrogate entered it; the
measured column comes from explicit Hamiltonian Monte Carlo sampling of each
perturbed potential, 3000 frames each. Across all eight designed fields the
root-mean-square residual between prediction and measurement is $1.01\sigma$
(Fig.~\ref{fig:prediction}, $n = 8$). The prediction agrees with the measurement
at exactly the level the error bars claim, neither better nor worse. This is the
practical payoff: evaluating a model's effect on an observable costs one energy
evaluation per stored frame rather than a full simulation.
...
second order, and Eq.~\eqref{eq:cumulants} predicts it. The second-order
estimates are $+0.021 \pm 0.023$ pairs at the lower force level and
$+0.332 \pm 0.374$ at the upper. Subtracting the first- and second-order terms
from the measurements leaves $-0.540 \pm 0.580$ and $-0.439 \pm 0.640$ pairs,
both consistent with zero. The formula is not merely right at leading order; its
own correction term is right too, which is what licenses the use of that term as
the self-diagnostic of Sec.~\ref{sec:cumulant}(iii).

How much data does the null-space construction need? Table~\ref{tab:budget}
gives the answer. Constructed on 250, 500, 1000 and 2000 frames and evaluated on
disjoint frames, the out-of-sample predicted shift is $0.041 \pm 0.067$,
$0.142 \pm 0.071$, $0.037 \pm 0.077$ and $0.028 \pm 0.078$ pairs, against the
in-sample values of order $10^{-15}$ that the construction guarantees. Set
against the aligned field's $10.206$ pairs at the same force error, the
suppression is roughly $400\times$, and it holds out of sample. An earlier
attempt using 30 construction frames gave 1.2 to 1.6 pairs: the null space of a
covariance estimated from 30 samples is the null space of the noise. The
construction is only as good as the covariance estimate, and
Table~\ref{tab:budget} is where that stops mattering.
...
Figure~\ref{fig:counterexamples} shows the full difference curve for the null,
aligned and random fields at both force levels. At $4\times10^{-3}~\eVA$ the
null field leaves its target bin alone at $-0.220 \pm 0.640$ pairs and moves the
largest non-target bin by $11.36 \pm 0.95$ pairs, which is as much as the
aligned field moves its own largest non-target bin, $11.18 \pm 1.21$ pairs.

This is a limit on the claim as much as it is a result. What
Sec.~\ref{sec:counterexamples} demonstrates is that force RMSE fails to predict
a \emph{particular} observable, not that any model is globally harmless. The
null field is not a benign potential; it is a potential that has been made blind
in one direction and is as damaging as its aligned counterpart in others.
Harmlessness is a relation between an error and a question, not a property of a
model. It follows that no single scalar can summarise model quality --- not
force RMSE and not any replacement for it, because a scalar is a projection onto
one direction and there are as many directions as there are questions. What can
be computed cheaply is a \emph{vector} of response scores, one per observable of
interest, at one energy evaluation per stored frame per model.
...
\noindent The pre-registered falsification criterion attached to it was that
force RMSE should rank-correlate strongly, with Spearman $\rho > 0.9$, against
observable error across a diverse zoo.
...
amplitudes; four high-frequency oscillatory fields; eight random fields at two
force levels; and the null and aligned fields at two force levels. The
observable is the full eight-bin difference curve and the observable error is
its norm. Across the whole zoo the Spearman rank correlation between force RMSE
...
\textbf{P1 as stated is refuted.} A rank correlation of 0.83 with an interval
whose lower end is 0.67 is not a weak proxy, and the prediction we registered is
wrong. The separate and stricter criterion we also registered --- that force
RMSE would have to reach $\rho > 0.9$ against \emph{every} observable error for
the whole argument to collapse --- is a different test, and the interval here
includes 0.9, so that test is not decided by this measurement. Both statements
are ours and we report both.

What replaces P1 is sharper. Force RMSE cannot choose among models of
\emph{comparable} force error, which is the only situation a practitioner is
ever in. Restricting to the 15 zoo members whose force RMSE lies in the window
$1.5$ to $6.0\times10^{-3}~\eVA$ --- a window chosen by the analyst after
seeing the data, hard-coded in the figure script, and not pre-registered ---
force RMSE spans a factor of 2.59 while the observable error still spans a
factor of 30. Inside that band force RMSE gives $\rho = 0.34$ [$-0.26$, 0.77],
$n = 15$, an interval that includes zero, while the response prediction of
Eq.~\eqref{eq:linear} gives $\rho = 0.94$ [0.76, 1.00], $n = 15$, an interval
that does not [Fig.~\ref{fig:regimes}(b)]. We are careful about what this
licenses: force RMSE is \emph{not resolved from zero} inside the band. It is not
demonstrated to be zero, and 15 members is not many. The contrast that survives
is between one predictor whose interval excludes zero and another whose does
not.
...
Sec.~\ref{sec:counterexamples} show why: within a band the ordering is set by
the projection, which force error does not measure.

Two secondary findings come from the same run and are visible in
Fig.~\ref{fig:proxies}(c). First, \textbf{the smoothness ratio proposed in this
work is refuted}. The quantity $\sigma_0(\dU)/\mathrm{RMSE}_F$ was proposed by
us as an empirical stand-in for the inverse spectral weighting of
Sec.~\ref{sec:frequency}; across the zoo it gives $\rho = -0.02$ [$-0.37$,
0.35], $n = 34$, and on the raw series $\rho = -0.01$ [$-0.37$, 0.37], $n = 34$.
It carries no information here. Proposed and refuted in the same study; see
Sec.~\ref{sec:smoothness}. Second, the per-atom energy spread outranks every
force metric, $\rho = 0.92$ [0.82, 0.97], $n = 34$, against 0.83 for force RMSE.
The theory does not predict this. A plausible reading is that the per-atom
energy spread tracks $\sigma_0(\dU)$, the second factor in the Cauchy--Schwarz
bound of Eq.~\eqref{eq:cs}, and so captures the magnitude factor that force RMSE
measures only through a gradient --- but we label that a conjecture rather than
a result, since nothing here tests it.

Top-$k$ overlap statistics between proxy rankings and the truth ranking were
also computed. They are withheld: with 34 members they moved between 0.33 and
0.67 depending on the noise-subtraction convention, which means they are too
...
Table~\ref{tab:zoo} lists the ten models of Sec.~\ref{sec:zoo}, their force
...
measured shift across the ten is $1.06\sigma$, matching the $1.01\sigma$
obtained on the designed fields. This is the external-validity check for
Sec.~\ref{sec:prediction} and it passes. The second-order self-diagnostic is
recorded for every model in this arm and flags 8 of the 10 as
...
$6.1\times10^{-3}~\eVA$ across the ten models and buys almost no measurable
difference in the observable, whose error spans 0.049 to 4.10 pairs with most
values at the noise floor. On this reference potential, at this data budget,
nearly every architecture is good enough that its observable error is
indistinguishable from zero. Every confidence interval from this arm spans zero:
force RMSE $\rho = 0.43$ [$-0.35$, 0.96], force MAE $\rho = 0.35$ [$-0.49$,
0.92], energy RMSE $\rho = 0.05$ [$-0.77$, 0.76], the smoothness ratio
$\rho = -0.53$ [$-0.86$, 0.06], the response prediction $\rho = 0.18$ [$-0.70$,
0.83], all $n = 10$. Nothing about ranking can be concluded from this arm, and
nothing is. It establishes exactly one thing, the $1.06\sigma$ residual.

One pair in the zoo is worth stating on its own, with its weakness attached.
\texttt{egnn\_c8\_s0} and \texttt{egnn\_c8\_s1} are the same architecture, the
same training data and the same hyperparameters, differing only in
initialisation seed. Their force RMSE differs by 36\%, $0.0045$ against
$0.0061~\eVA$. Their measured target-bin errors are $-0.15 \pm 0.57$ pairs
($0.3\sigma$, indistinguishable from zero) and $-4.10 \pm 0.69$ pairs
($6.0\sigma$), a factor of 27. Both were predicted from the reference trajectory
alone, at $-0.98$ and $-3.03$ pairs. Anyone selecting between these two on force
RMSE would see a 36\% difference and reasonably call them equivalent.
\textbf{This is $n = 2$.} It is the most quotable observation in the paper and
its weakest evidence, and the two facts belong in the same sentence. Seed
...
comparison at $n \ge 8$ seeds is the highest-value follow-up we can name.

Both EGNN records carry \texttt{linear\_trustworthy = false}, with second-order
ratios of 0.41 and 0.14 against a median of 0.015 across the other eight models.
The self-diagnostic of Sec.~\ref{sec:cumulant}(iii) fired on exactly the two
models where the linear prediction is least reliable, and the predictions for
those two are correspondingly quoted here as an ordering --- the second seed is
predicted to be worse, and is --- rather than as precise values.
...
Figure~\ref{fig:mechanism}(b) shows the result. The measured norm of the
difference curve spans a factor of 4.66, from $2.13 \pm 2.15$ pairs at
$w = 0.10$~\AA{} to $9.95 \pm 2.47$ pairs at $w = 0.65$~\AA. The first-order
prediction tracks the same rise across the range, from 3.22 to 9.68 pairs, with
much smaller uncertainty; the direct measurements at the small-$w$ end carry
error bars comparable to their own values, so the ordering across the sweep is
carried mainly by the prediction, and the honest statement of the measurement is
...
Section~\ref{sec:retraction} predicted, and then retracted on analytic grounds,
that at fixed force error the observable damage would grow as $w^{3/2}$. The
measurement on the sweep of Sec.~\ref{sec:width}, on a
verified-stationary ensemble, gives a fitted exponent of \textbf{0.56} from the
direct sampling and \textbf{0.45} from the first-order prediction
[Fig.~\ref{fig:response}(b)], against the predicted 1.5. The estimate is wrong
by a factor of three in the exponent.

The retraction was made analytically and before the measurement, for the two
reasons given in Sec.~\ref{sec:retraction}, so this is not a prediction quietly
withdrawn once it failed --- but it did fail, and a heuristic that survives its
own author's scepticism only to be refuted by measurement is worth reporting as
a refutation. What survives is the
weaker and still sufficient statement that force error and observable error
depend differently on the shape of the error field, so their ratio is not a
constant; Sec.~\ref{sec:width} measures that ratio varying by 4.7 at exactly
fixed force RMSE, and that is the statement the argument of this paper actually
needs.

\subsection{The proposed smoothness-ratio metric is refuted}
\label{sec:smoothness}

If force RMSE weights the error spectrum by $k^2$ and observables weight it by
roughly $k^0$, then a ratio of an error-field magnitude to a force error should
carry information about where in the spectrum a model's error sits. We proposed
$\sigma_0(\dU)/\mathrm{RMSE}_F$ on exactly that reasoning. Measured against
observable error across the 34-member zoo it gives $\rho = -0.02$ [$-0.37$,
0.35], $n = 34$ on the noise-subtracted series and $\rho = -0.01$ [$-0.37$,
0.37], $n = 34$ on the raw series. On either convention it carries no
...
Sec.~\ref{sec:regimes} gives $\rho = 0.84$ [0.49, 0.98], $n = 15$. We do not
present that as support for the metric: the band is analyst-chosen after the
fact, the members inside it are dominated by one family of shell perturbations,
...
\subsection{An open 35\% discrepancy, with three explanations excluded}
...
in the peak bin disagreed: first order $-3.835 \pm 0.170$ pairs, exact
reweighting $-3.884$ pairs at an effective-sample-size fraction\cite{Kish1965}
of 0.83, and direct sampling $-1.948 \pm 0.821$ pairs
...
estimator can see. \textbf{We tested that diagnosis and it was wrong.} Sampling
both potentials from six independent seeds each gives a within-chain blocking
error of 0.589 pairs against a between-chain scatter of 0.312 pairs, a ratio of
0.53: blocking is \emph{conservative} here by about a factor of two, not
optimistic. The six independent seed pairs give measured shifts of
$[2.553, 2.773, 2.349, 2.611, 1.788, 2.188]$ pairs, mean
$+2.377 \pm 0.144$ ($n = 6$). This check was run on the same perturbation but on
a different bin --- the one centred at 5.25~\AA{} rather than the peak bin at
4.25~\AA{} --- so the sign and size differ from the row above; what transfers is
the size of the between-chain offset and the reliability of the direct estimate.
...
$+1.725 \pm 0.101$ pairs (first order) and $+1.757$ pairs (exact reweighting),
against the direct $+2.377 \pm 0.144$: a difference of 0.652 pairs, 35\% of the
reweighted value and 38\% of the first-order value, and $3.7\sigma$ on the
combined uncertainty.

Three explanations are excluded by measurement. \emph{Not the error bars}: the
between-chain scatter is half the quoted within-chain error, not larger. \emph{Not
second-order truncation}: the first-order estimate and the all-orders
reweighting identity agree with each other to 2\%, so the truncation cannot be
the difference. \emph{Not incomplete relaxation of the perturbed chain}: that
would bias the measured shift \emph{toward} the reference and make it smaller in
magnitude, and it is larger.

What remains is a candidate we did not test. The Kish effective sample
size\cite{Kish1965} used to certify the reweighting estimate assumes independent
samples, and the true independent count in a correlated chain is smaller by the
integrated autocorrelation time; recomputing the effective size as
$M/(2\tau_{\rm int})$, with the $\tau_{\rm int}$ the codebase already estimates,
is the obvious next test and was not run. We record the item as unresolved
rather than attributing it to the nearest plausible cause. It is not a general
failure of either estimator: the measurements of
Sec.~\ref{sec:counterexamples} used a different observable and twice as many
frames and are calibrated at $1.01\sigma$ against the same prediction.
...
reduced quick-run mode, puts the failure on the record
(Table~\ref{tab:breakdown}). Force RMSE there reaches $4.34\times10^{-2}~\eVA$
and observable errors reach 126 pairs; the aggregate prediction residual is
...
$E(3)$-equivariant model at $4.34\times10^{-2}~\eVA$, whose predicted shift of
$-14.2 \pm 7.3$ pairs faces a measured $-126.16 \pm 1.85$. The most instructive
row is the mildest: \texttt{spline\_k12}, at force RMSE
$1.55\times10^{-3}~\eVA$, has a predicted shift of $0.026 \pm 0.251$ pairs,
consistent with zero, against a measured $-5.173 \pm 1.863$ pairs. A small
force error and a near-zero predicted response are together not a guarantee.

This is the demonstrated boundary of validity of Eq.~\eqref{eq:linear}, reported
as a result and not as a hidden failure. It carries a gap in our evidence that
we state in those words: \textbf{the records from this arm do not contain the
\texttt{second\_order\_ratio} or \texttt{linear\_trustworthy} fields, so the
self-diagnostic of Sec.~\ref{sec:cumulant}(iii) cannot be checked against this
arm.} The claim that the second-order term gives advance warning of exactly this
breakdown is supported by the amplitude sweep of Fig.~\ref{fig:response}(a),
where the diagnostic flags the two smallest amplitudes as trustworthy and the
five larger ones as not, and by the fitted zoo of Sec.~\ref{sec:fitted}, where
it flags the two EGNN seeds; it is not supported by the arm where the breakdown
is largest, because the field was not recorded there.
...
so that a perturbation $\delta u$ of the pair potential shifts the bin counts by
...
exact reweighting identity agree with each other to within 0.003 pairs in every
bin, at most 1.5\% of the shift in any bin where the shift is resolved. Both
disagree with our hand-derived closed form, by $4.7\sigma$ root-mean-square and
$6.5\sigma$ at worst over seven bins.
...
that normalisation term gives a closed form wrong by a factor of ten in the
outer bins and wrong in sign in the tail. A covariance is mean-subtracted, and
the subtraction \emph{is} the normalisation, so the estimator never had the
problem --- a small illustration of why an estimator that computes the right
object is safer than a derivation that reconstructs it.
...
benchmarked machine-learned force fields on simulation tasks rather than on
held-out forces and found the ranking by force error to be a poor guide to the
ranking by simulation outcome; Stocker \emph{et al.}\cite{Stocker2022} found
graph-network potentials with excellent test errors to be unreliable in long,
hot molecular dynamics. This paper does not claim to have reproduced either
benchmark: no published potential and no dynamical metric was evaluated here.
What it adds is threefold. \emph{(i) Confound removal.} In those comparisons
models differ simultaneously in force error, architecture, training data and
--- decisively --- the \emph{magnitude} of their error, so shape and size cannot
be separated. Here the magnitude is held fixed by construction, to 0.5\% in
force RMSE, and only the shape varies. \emph{(ii) A mechanism.} Those studies
offer an observation; Eq.~\eqref{eq:linear} supplies a mechanism and we validate
it at $1.01\sigma$ ($n = 8$) and $1.06\sigma$ ($n = 10$). \emph{(iii) A
different disease.} Their failure mode is dynamical instability: large $\dU$,
out of distribution, non-perturbative, and visible --- simulations that melt,
...
and the answer is wrong by $16\sigma$. These are different diseases with
different remedies, and conflating them would understate both.

\paragraph*{The identity is not new.} We say this plainly because the
...
the practical machinery around it. The same covariance is used as a parameter
...
uncertainty in the potential through to uncertainties in thermodynamic averages.
...
predicted and not a single number attached to the model. What is new here is the
...
al.}\cite{Wu2024} derive a correction to lattice thermal conductivity computed
with machine-learned potentials in which the force error enters through a norm,
and correct a systematic underestimation with it. That is the expected structure
...
static observables is exactly the asymmetry conjectured in
Sec.~\ref{sec:dynamics}, and Ref.~\onlinecite{Wu2024} is independent evidence
for it --- not evidence produced by this work, which tested no dynamical
...
held-out errors alone. This paper supports that recommendation quantitatively
...
recommended by this work is a \emph{vector} of response scores --- one
$-\beta\,\Cov_0(A_k,\dU)$ per observable of interest --- computed at one energy
evaluation per stored frame per model on an existing reference trajectory. The
honest caveat is that a practitioner has no $U_0$, so $\dU$ is not available.
...
from consensus whose spread across the committee is an uncertainty on the
observable itself. Its failure mode is predictable and one-directional:
homogeneous committees share systematic error, and shared error cancels exactly
...
and deliberately heterogeneous committee arms, was not run, and the committee
predictor is a hypothesis in this paper and not a finding.
...
point, $N = 108$, $\rho^* = 0.790$, $T^* = 1.004$. The choice of an analytic
reference is the methodological crux of the paper --- it is what makes $\dU$
exactly known and error fields designable --- and it is simultaneously the
...
machine-learned potential was evaluated; the ten fitted models are deliberately
...
structure, which is precisely where real architectures differ from one another.
No dynamical observable of any kind: no diffusivity, no vibrational spectrum, no
melting point, no elastic constant of any surrogate model was measured, and no
claim about any of them is made.

\emph{By resources.} No finite-size study: every number is $N = 108$, and we do
not assert that the covariance mechanism is size-independent. The check is
...
that the covariance in Eq.~\eqref{eq:linear} inherits. The seed-pair observation
of Sec.~\ref{sec:fitted} is $n = 2$ and does not generalise as stated; it is an
...
seeing the data and is hard-coded in the figure script, and the 15 members
inside it were not pre-registered. The dynamical conjecture of
Sec.~\ref{sec:dynamics} is untested here. The committee predictor of
Sec.~\ref{sec:prior} is untested here. The remaining candidate explanation for
the discrepancy of Sec.~\ref{sec:discrepancy} is untested. The quarter-density
repeat that would confirm the reading of Sec.~\ref{sec:dilute} was not run. The
large-$\dU$ arm of Sec.~\ref{sec:breakdown} lacks the second-order diagnostic
field, so the warning-light claim cannot be checked where the breakdown is
largest.
...
three decades. The claim is about resolution, not about information content.
...
decades, $\rho = 0.83$ [0.67, 0.93], $n = 34$, and is not resolved from zero
inside a factor-2.6 band where the physics still varies by $30\times$,
$\rho = 0.34$ [$-0.26$, 0.77], $n = 15$. The response prediction is resolved
there, $\rho = 0.94$ [0.76, 1.00], $n = 15$. Two further predictions of ours
failed outright: a smoothness-ratio metric we proposed carries no information,
$\rho = -0.02$ [$-0.37$, 0.35], $n = 34$, and a $w^{3/2}$ scaling we derived has
a measured exponent of 0.56.
...
projection; a norm is not. At force RMSE matched to 0.5\% we moved a target pair
count from $-0.220 \pm 0.640$ to $+9.891 \pm 0.611$ pairs by changing only the
correlation between the error field and the observable, and the null-space
construction that achieves the first of those survives out of sample with a
$400\times$ suppression relative to its aligned counterpart. The same field
moves a non-target bin by $11.36 \pm 0.95$ pairs, so harmlessness is a relation
between an error and a question, not a property of a model.
...
over ten fitted models, at one energy evaluation per stored frame per model, and
its own second-order term gives advance warning where it fails. What we
recommend reporting is therefore not a better scalar but a vector: one response
...
$4.0\times10^{-3}$ & null            & $4.00217\times10^{-3}$ & $+0.009$ & $-0.113$ & $0.333$ & $-0.220$ & $0.640$ & 0.3 \\
                   ...
                   & random (seed 0) & $3.99100\times10^{-3}$ & $-0.430$ & $+5.778$ & $0.418$ & $+5.320$ & $0.589$ & 9.0 \\
