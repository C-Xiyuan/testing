...
root-mean-square residuals of $1.01\sigma$ over eight designed error fields
($n = 8$), $1.06\sigma$ over ten fitted models ($n = 10$) and $1.00\sigma$ over
eight more fitted at a smaller data budget ($n = 8$), at a cost of one energy
evaluation per stored frame per model and no simulation with the surrogate. In
the two eight-member arms every residual has the same sign, and we decompose the
figure into a common offset and the scatter about it rather than quoting it as a
calibration (Secs.~\ref{sec:prediction} and \ref{sec:fitted}).
\item A constructive counterexample set: four error fields matched in force RMSE
to 0.5\%, whose target-observable shifts run from $-0.22 \pm 0.64$ to
...
361-fold range of force RMSE, unresolved within a narrow band where the physics
...
caveats (Sec.~\ref{sec:failures}), together with a boundary of
...
perturbation and no linear reasoning about it is safe --- including the field's
own implicit assumption that a little more accuracy helps a little. We state
this as a hypothesis rather than a result, because our own tests of it are
mixed: Sec.~\ref{sec:breakdown} reports a sweep in which one of the two cases
the diagnostic certified was in fact the largest unexplained disagreement in
this paper. We report the diagnostic wherever it was recorded, report its
...
$1.8\times10^{-4}~\eVA$, delivering an impulse every time a pair crosses the
...
acceptance rate and never as a biased distribution. That is the reason for the
...
between two ensembles that this study attributes to model error. Production
...
$Q_6 \approx 0.08$ and perfect fcc gives 0.5745. After the protocol,
...
it; Sec.~\ref{sec:fitted} reports the one occasion on which it fired. The
quick-run mode that shrinks every other size parameter never shortens
equilibration.
...
7.0~\AA{} (centres 3.25, 3.75, \dots, 6.75~\AA). They differ from the radial
distribution function $g(r)$ in that bin only by a
constant at fixed $N$ and $V$\cite{AllenTildesley2017}; the analysis is done on
the counts because response theory applies to a plain function of configuration,
and a bin count is one, with no normalisation between predicted and measured.
...
count between 3.4 and 3.9~\AA. That bin is deliberately
offset from the eight-bin grid, so a shift in it cannot be read off the
difference curve. Targeting a scalar makes the covariance a $K$-vector rather
...
parallel to it, is then linear algebra rather than optimisation, giving three
families: \emph{null}, whose first-order predicted effect on
the target observable is zero by construction; \emph{aligned}, of maximal effect
per unit force error; and \emph{random}, the control. All are scaled to the same
force RMSE on the reference ensemble, so the number a practitioner would report
is held fixed across the comparison.

Two null constructions appear below: the \emph{scalar-null} field, orthogonal to
the single target bin, and the \emph{curve-null} field, orthogonal to all eight
bins at once.
...
is computed on the other. How many construction frames are needed is itself
...
Two further arms at smaller data budgets are reported in
...
report them anyway, because their force errors are the smallest in the zoo and
...
a factor of several for a typical liquid observable.
...
\overline{\dU})$; the observable's own correlation time underestimates the
block length whenever $\dU$ varies slowly, which is the systematic-error case
this paper is about. A jackknife with a different bias
structure is a cross-check; Appendix~\ref{app:statistics} gives the
...
Prediction residuals in $\sigma$ are the measured minus predicted shift divided
...
and the seed. Seeds derive from a single experiment seed through named
\texttt{SeedSequence} sub-streams, so inserting a stage does not perturb every
later result.

We state what those manifests record, because this paper is about auditability.
The six runs reported here come from five distinct commits, and \textbf{every
one was produced from a dirty working tree}, so no number here can be
regenerated from a named revision; a clean re-run from a tagged commit is the
...
not the processor model, so we state none. The
...
between 3.4 and 3.9~\AA, and each was scaled to a prescribed force RMSE at two
...
the construction --- the four fields agree in force RMSE to within 0.52\% at
each level, running from $0.99535$ to $1.00054 \times 10^{-3}~\eVA$ at
...
$+9.89 \pm 0.61$ pairs for the aligned field, $16.2\sigma$ from zero.
The two random controls sit between them, at $-1.48 \pm 0.58$ ($2.6\sigma$)
and $+5.32 \pm 0.59$ ($9.0\sigma$). At the lower level the same four fields
give $-0.55 \pm 0.58$ ($0.9\sigma$), $-1.45 \pm 0.58$ ($2.5\sigma$),
$+0.84 \pm 0.59$ ($1.4\sigma$) and $+1.83 \pm 0.58$ ($3.2\sigma$). The
lower-level measurements are not the upper-level ones divided
by four: the \emph{predictions} scale almost exactly with the force level (2.55
against 10.21 pairs for the aligned field) because they are linear in $\dU$,
...
$10.11$ pairs above the scalar-null field. Combining the two tabulated errors in
quadrature double-counts the shared reference term and so overstates the
uncertainty; even on that conservative footing the separation is $11.4\sigma$,
and the true figure is larger.

Two conclusions follow. First, the reported
metric cannot distinguish these four models. Their force RMSE agrees to 0.52\%
...
first two factors are held nearly fixed. This is an existence proof, not a
frequency claim: it shows that force RMSE \emph{can} be blind to a large
observable error, not how often fitting produces error fields with weight in
that blind direction.
